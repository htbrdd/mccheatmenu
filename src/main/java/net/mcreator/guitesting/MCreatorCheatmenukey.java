...
public class MCreatorCheatmenukey extends Elementsguitesting.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public MCreatorCheatmenukey(Elementsguitesting instance) {
		super(instance, 4);
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (!(Minecraft.getInstance().currentScreen instanceof ChatScreen)) {
			if (event.getKey() == keys.getKey().getKeyCode() && event.getAction() == GLFW.GLFW_PRESS) {
				guitesting.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
